      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span
              className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-[10px] ${
                filter.enabled
          ...
          {targetHosts.length > 0 && (
            <div className="text-[11px] text-gray-600 mt-1 flex flex-wrap gap-1">
              {targetHosts.map((h) => (
                <span
                  key={h.id}
                  className="px-1.5 py-0.5 bg-gray-100 border border-gray-200 rounded-[10px]"
                >
                  {h.primary_name}
                </span>
              ))}
        ...
        <div className="flex gap-2 shrink-0">
          <button
            ...
            className="text-[11px] uppercase tracking-wider px-3 py-1.5 border border-gray-300 hover:border-gray-400 text-gray-700 rounded-[10px] transition-colors"
          >
          ...
          <button
            ...
            disabled={busy}
            className="text-[11px] uppercase tracking-wider px-3 py-1.5 border border-gray-300 hover:border-gray-400 text-gray-700 rounded-[10px] transition-colors disabled:opacity-40"
          >
            {busy ? '…' : filter.enabled ? 'Disattiva' : 'Attiva'}
          </button>
          <button
            onClick={onEdit}
            className="text-[11px] uppercase tracking-wider px-3 py-1.5 border border-gray-300 hover:border-black hover:bg-black hover:text-white text-gray-700 rounded-[10px] transition-colors"
          >
            Modifica
          </button>
          <button
            onClick={onDelete}
            disabled={busy}
            className="text-[11px] uppercase tracking-wider px-3 py-1.5 border border-gray-300 hover:border-red-600 hover:text-red-700 text-gray-700 rounded-[10px] transition-colors disabled:opacity-40"
          >
            Elimina
          </button>
        </div>
